...
int ver,l,ll,ls;				//Cheat and 3 types of cheating
...
int readavf()
{
    ...
    //For example, Arbiter 0.52.2 stores 52 (or Hex 34) in byte 1
	c=_fgetc(AVF);

	//Check if video is Freesweeper instead of Arbiter
	//If version has a value then it is Arbiter otherwise Freesweeper
	fs=!(ver=c);
	...
	//Check if it is a Freesweeper video
	if(fs)
	{
		for(i=0;i<cur;++i)
		{
			//Freesweeper records 3 decimal places
...
void writetxt()
{
	...
	//This is a Freesweeper video
	else printf("Version: R%d\n",ver);
